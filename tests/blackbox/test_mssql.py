...
async def test_local_connection_async(docker_client):
    ...
    async with MSSQLService.arun(
        docker_client, image="mcr.microsoft.com/mssql/server:2022-latest"
    ) as service:
...
def service(docker_client):
    with MSSQLService.run(
        docker_client, image="mcr.microsoft.com/mssql/server:2022-latest", spinner=False
    ) as service:
        yield service
...
def test_remote_connection_string_host(
    service, db, engine, create_and_pull, docker_client
):
    with engine.begin() as connection:
        connection.execute(
            text(
